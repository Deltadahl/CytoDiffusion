...
def check_and_rename_folder(source_path):
    ...
    def load_name_to_number(file_path):
        ...
        name_to_number = {k.lower(): v for k, v in name_to_number.items()}

        # Determine the number of unique classes
        unique_classes = set(name_to_number.values())
        n_classes = len(unique_classes)

        # Create folder_to_number dictionary
        n_classes = max(name_to_number.values()) + 1
        folder_to_number = {str(i): i for i in range(n_classes)}

        return name_to_number, folder_to_number

    # Usage
    json_file_path = os.path.join(source_path, "name_to_number.json")
    name_to_number, folder_to_number = load_name_to_number(json_file_path)

    print("name_to_number:", name_to_number)
    print("folder_to_number:", folder_to_number)
    ...
    for subfolder in source_path.iterdir():
        if subfolder.is_dir():
            ...
            images = [
                f for f in subfolder.iterdir() if f.suffix.lower() in image_extensions
            ]

            print(f"Found {len(images)} images in {subfolder_name}")  # Debug print
            ...
            for image in tqdm(images, desc=f"Processing {subfolder_name}"):
                ...
                json_file_path = (
                    json_path / new_name / f"{unique_filename.split('.')[0]}.json"
                )
    ...
    print(
        "Folder structure checking, renaming, and JSON file creation completed successfully."
    )
...
def create_splits_percentage(source_dir, train_percent, val_percent):
    ...
    for cls in classes:
        ...
        print(
            f"Num train files: {len(train_files)}, Num val files: {len(val_files)}, Num test files: {len(test_files)}"
        )
...
def create_splits_fixed(source_dir, train_count, val_count, test_count):
    ...
    for cls in classes:
        ...
        if len(files) < total_required:
            print(
                f"Warning: Not enough images in class {cls}. Using all available images."
            )
            train_files = files[: min(train_count, len(files))]
            val_files = files[
                len(train_files) : min(len(train_files) + val_count, len(files))
            ]
        ...
        for name in files:
            if name.lower().endswith(
                ...
                json_file_path = os.path.join(
                    numeric_folder_path, relative_path, name.split(".")[0] + ".json"
                )
...
def process_images(image_files, image_h_and_w):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(load_image, file_path, json_path, image_h_and_w): (
                file_path,
                json_path,
            )
...
def create_arrow_data(
    train_dir, numeric_folder_path, image_h_and_w=IMAGE_RESIZE_SIZE, chunk_size=30000
):
    ...
    for index, chunk in enumerate(chunks):
        ...
        print(
            f"Creating .arrow file {index+1}/{len(chunks)}. This will take a while (~ {approximated_time_it_will_take:.2f} hours.)"
        )
    ...
    merged_dataset_dict.save_to_disk(
        os.path.join(base_path, "dataset_with_text_MERGED")
    )
...
if __name__ == "__main__":
    ...
    source_path = input(
        "Enter the path to the folder containing subfolders with images: "
    )
    split_method = input(
        "Choose splitting method (1 for percentage-based, 2 for fixed number per class): "
    )

    if split_method == "1":
        train_percent = (
            float(input("Enter the percentage for train split (e.g. 70): ")) / 100
        )
        val_percent = (
            float(
                input(
                    "Enter the percentage for validation split (the rest will be used in the test set) (e.g. 10): "
                )
            )
            / 100
        )
        ...
        train_count = int(
            input("Enter the number of images per class for train split: ")
        )
        val_count = int(
            input("Enter the number of images per class for validation split: ")
        )
        test_count = int(input("Enter the number of images per class for test split: "))
    else:
        print("Invalid choice. Exiting.")
        exit()

    # Now execute the code based on the inputs
    source_path = Path(source_path)
