...
export new_train_steps=72000
export total_steps=$((new_train_steps + previous_train_steps))
export output_dir="saved_models/${total_steps}-in-domain-PBC"
...
accelerate launch --mixed_precision="fp16"  diffusion_discriminator.py \
  ...
  --snr_gamma=5.0 \
  --noise_offset=0.1 \
  --validation_img_dir="../data/datasets/PBC_dataset_normal_DIB_train_val_test/val" \
