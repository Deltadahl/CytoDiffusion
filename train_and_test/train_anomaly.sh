...
export new_train_steps=140000
export total_steps=$((new_train_steps + previous_train_steps))
export output_dir="saved_models/${total_steps}-anomaly-erythroblast"
...
accelerate launch --mixed_precision="fp16"  diffusion_discriminator.py \
  ...
  --snr_gamma=5.0 \
  --noise_offset=0.1 \
  --validation_img_dir="../data/anomaly_erythroblast/val" \
  --validation_prompt_dir="../data/anomaly_erythroblast/val_json" \
  --random_flip \
